...
class BundleManagerTest extends PHPUnit_Framework_TestCase
{
     ...
     * @covers \MockServer\Manager\BundleManager
     */
    public function testBundleManagerGeneratesSecurityYml()
    {
        $expectedContent = $this->generator->render($this->skeletonDirectory, 'security.yml', $this->parameters);

        $bundleManager = new BundleManager(new Filesystem(), $this->skeletonDirectory);
        $bundleManager->generate($this->parameters['namespace'], $this->parameters['bundle'], 'mfs://');

        $this->assertSame($expectedContent, $this->mockFs->getFileSystem()->getChildByPath('/MockNamespace/app/config/security.yml')->getContents());
    }
}
